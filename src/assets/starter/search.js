(function() {
  ...
  async function prepareSearch() {
    ...
    try {
      ...
      searchIndex.forEach((item, index) => {
        if (item && item.title) {
          const searchText = `${item.title} ${item.seoTitle || ''} ${item.description || ''} ${item.searchIntent || ''} ${item.content || ''} ${(item.tags || []).join(' ')}`;
  ...
  function createSearchResultsContainer(searchForm) {
