...
That is the recipe for a much more useful assistant. It is also the recipe for a much larger prompt injection surface.
...
That is the most obvious version, but it is not the version I worry about most.
...
The [UK National Cyber Security Centre](https://www.ncsc.gov.uk/blog-post/prompt-injection-is-not-sql-injection) has made the point clearly: LLMs do not have a clean internal separation between data and instructions the way traditional systems try to separate commands from input. In an LLM, everything is part of the token stream. That is why prompt injection may not be “fixed” the same way SQL injection was eventually reduced with safer query handling.

That matters because Siri AI is not being built as a single chat box. It is being integrated into the OS.
...
This is the part users need to understand.
...
This is why prompt injection is so annoying. It lives at the boundary between content and command. Traditional security controls are good at detecting known malware, network indicators, and suspicious binaries. They are not naturally built to detect a sentence that manipulates an AI assistant.
...
[OWASP’s Excessive Agency guidance](https://genai.owasp.org/llmrisk/llm062025-excessive-agency/) says damaging actions can happen when LLM-based systems have excessive functionality, excessive permissions, or excessive autonomy. That is exactly the lens to use here. A prompt injection that changes a summary is one level of concern. A prompt injection that influences a file share, email response, calendar change, Shortcut, website account change, or app action is another level entirely.

That is why “human confirmation” matters, but it cannot be lazy confirmation. A vague “Do you want to continue?” is weak. A useful confirmation should show the exact action, the target, the data being shared, and any security-sensitive side effects.
...
Apple is pushing Visual Intelligence across platforms. That means the camera, screenshots, images, and on-screen content can become AI context. This is useful for accessibility, learning, shopping, troubleshooting, and productivity. It also means the physical world can become a prompt surface.
...
This is not me saying “never use Visual Intelligence.” I am saying we need to understand that the camera is no longer just a camera. It can become an instruction input.
