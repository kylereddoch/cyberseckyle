...
The interesting security question is not whether the feature is useful. It is whether Apple can keep the workflow narrow enough that "change this weak password" does not quietly become "let an agent improvise inside my account settings."

## What Apple Has Actually Said

Apple has not published a full technical design for this feature yet. As of July 1, 2026, the public description is a short product announcement, not a security architecture document. The security question is hiding in the word "eligible."
...
I do not know which design Apple has chosen. The public details are not enough to say, so I would rather evaluate the boundaries than score the announcement on vibes.
...
The way to make this safer is to keep the job deliberately small.
...
"On device" helps with one part of the trust problem, but it does not settle the whole thing. Local processing can reduce exposure to a cloud service, yet the workflow still touches browser state, web content, credential storage, autofill logic, and account recovery pages that Apple does not control.
...
The risk is not the cartoon version where a malicious page says "ignore previous instructions" and the phone immediately hands over every password. The more realistic problem is duller and harder to spot: browser agents have to interpret pages, and pages mix trusted account UI with marketing copy, banners, support widgets, redirects, ads, and user-generated content.
...
The job is to rotate the password under tightly bounded conditions. It should not become "secure the account," "fix whatever the page recommends," or "follow the site's instructions."
...
The joint [Five Eyes guidance on agentic AI](https://www.cisa.gov/resources-tools/resources/careful-adoption-agentic-ai-services) is relevant here. Agents that can take meaningful action need least privilege, human approval for high-impact steps, logging, monitoring, and fail-safe behavior when they are uncertain. A password change is a high-impact step. It may be routine, but it is still the secret that controls access to the account.
...
I would expect Apple to treat the password rotation like a small transaction:
...
Apple says this process can show up as a Live Activity. A sensitive background-ish action should be visible while it is happening.
...
Apple needs to make that boundary visible, because this feature will only earn trust if users can tell the difference between helpful automation and delegated account control.
