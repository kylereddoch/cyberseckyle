...
const dateForFeed = (date) => toDate(date).toISOString()

const isOldPost = (date) => {
  const d = DateTime.fromISO(toDate(date).toISOString()).setZone(ZONE)
...
const toDateTime = (date) =>
  DateTime.fromISO(toDate(date).toISOString()).setZone(ZONE).toFormat('MM/dd/yyyy h:mm:ss a')

const toDateTimeNoSeconds = (date) =>
  DateTime.fromISO(toDate(date).toISOString()).setZone(ZONE).toFormat('MM/dd/yyyy h:mm a')

const isoDateOnlyForDiscussion = (date) =>
  DateTime.fromISO(toDate(date).toISOString()).setZone(ZONE).toFormat('MM/dd/yyyy')
...
const postTime = (date) =>
  DateTime.fromISO(toDate(date).toISOString()).setZone(ZONE).toFormat('h:mm a') // switch to 'h:mm a' for US-style time
